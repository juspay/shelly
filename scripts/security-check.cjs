        ...
        allPatterns.forEach((rule) => {
          ...
          if (matches) {
            matches.forEach((match) => {
              ...
              // Skip false positives: UI prompt labels, console log help text,
              // and cross-line regex matches (match spans multiple lines)
              if (match.includes('\n')) return; // multi-line match = cross-boundary false positive
              const matchLine = lineNumber > 0 ? lines[lineNumber - 1] : '';
              if (
                matchLine.includes('message:') ||
                matchLine.includes('console.log') ||
                matchLine.includes('console.warn') ||
