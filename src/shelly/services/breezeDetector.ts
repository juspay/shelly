...
export class BreezeDetector {
  ...
  static async detectGitRemote(cwd: string): Promise<{
    ...
    try {
      ...
      // HTTPS public: https://bitbucket.org/workspace/repo.git
      // HTTPS Juspay: https://bitbucket.juspay.net/scm/workspace/repo.git  (3-segment /scm/ prefix)
      const sshMatch = remoteUrl.match(/git@[^:]+:([^/]+)\/([^/.]+)/);
      // Match /scm/workspace/repo (Juspay internal) or /workspace/repo (public Bitbucket)
      const httpsJuspayMatch = remoteUrl.match(
        /https?:\/\/[^/]+\/scm\/([^/]+)\/([^/.]+)/
      );
      const httpsMatch =
        httpsJuspayMatch ||
